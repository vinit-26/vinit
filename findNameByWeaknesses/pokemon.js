...
//end of pokemon data object

function pokemonWeaknes (weaknes , pokemonData)	//creating a function which takes the weaknes as input i.e pokemondWeaknes
{
						
	noWeaknessesfound = false;

	for(var x in pokemonData.pokemon)	//for loop to find the weaknes in pokemon array
{
		
		for(var y in pokemonData.pokemon[x].weaknesses)		//for loop to find the given weaknes in weaknesses array

{			
			if(pokemonData.pokemon[x].weaknesses[y] == weaknes)	//if condition to check which all pokemon have that weaknes 
{
								
				noWeaknessesfound = true;

				alert(pokemonData.pokemon[x].name,null);

		}
	}
}
				if(noWeaknessesfound == false)	//if condition if given weaknes is not available
{

					alert("No Pokemon has this weaknes")
	}
}

 var weaknesOfPokemon = window.prompt("Please enter the Weaknes"); 	//variable whhich takes the weaknes as input

 pokemonWeaknes(weaknesOfPokemon , pokemonData);	//calling the function
