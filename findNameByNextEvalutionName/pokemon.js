	// variable which holds the object
...
function pokemonName (entername , pokemonData)	//function which takes the pokemon name as input
	{
						
	noPokemonfound = false;

	for(var x in pokemonData.pokemon)	//for loop to find the given pokemon name in pokemon array
	
	{

		for(var y in pokemonData.pokemon[x].next_evolution)	//for loop to find the pokemon name in next_evolution field

			{

			if(pokemonData.pokemon[x].next_evolution[y].name == entername)	//if condition to check which all pokemon has that name in their next_evolution field
				...
				if(noPokemonfound == false)	//if condition if wrong pokemon name
					
					{

					alert("Pokemon not exist in next_evaluation list")
	}
}

 var nameOfPokemon = window.prompt("Please enter the name of the Pokemon");	//variable which takes the name as input

 pokemonName(nameOfPokemon , pokemonData);	//calling the function
